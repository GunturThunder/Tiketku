...
module.exports = {
  ...
  getAll: async (req, res) => {
    try {
      ...
      const result = await hotelModel.getAll(
        name,
        lowest,
        highest,
        city,
  ...
  createHotels: async (req, res) => {
    try {
      ...
      if (
        ...
        return res.json({
          message: "Not allowed upload another file except image"
        });
      }

      if (img.size > 5000000) {
        return res.json({ message: "Not allowed upload more than 5MB" });
      }
      ...
      const imageAccess = `/v1/images/${filename}`;
      const data = {
